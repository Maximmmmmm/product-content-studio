...
Completed:
- **`README.md`** — the last missing mandatory deliverable. Covers prerequisites, step-by-step
  setup, running both apps, running the tests, admin login, the demo data (including which
  product is the draft to check), project structure, the full API table with validation rules,
  copy-pasteable `curl` examples, security notes, actual verification results, known
  limitations, and a time-spent section.
- **`docs/TECHNICAL-REVIEW.md`** — an interview checklist covering architecture,
  authentication, authorization, validation, database design, API design, React architecture,
  testing, security and AI usage. Each section carries the questions an interviewer is most
  likely to ask and the honest answer, plus a closing list of things not to oversell.
- **`AI-WORKLOG.md`** — final summary completed: tools and models, AI contribution, candidate
  contribution, four concrete rejected/changed examples, how correctness was verified, lessons
  learned, and known limitations.
- **`docs/DECISIONS.md`** — every decision recorded with its rejected alternatives.

Verification — a genuine **clean-room run**. `backend/node_modules`, `backend/dist`,
`backend/prisma/dev.db`, `backend/.env`, `frontend/node_modules`, `frontend/.next` and
`frontend/.env.local` were all deleted, and the README was then followed verbatim:

| Step | Result |
|---|---|
| `cp .env.example .env` → `npm install` (backend) | installed; `postinstall` generated the Prisma client (confirmed `node_modules/.prisma/client` exists) |
| `npx prisma migrate deploy` | migration applied, `dev.db` created |
| `npm run prisma:seed` | 1 admin + 3 products (2 published, 1 draft) |
| `cp .env.example .env.local` → `npm install` (frontend) | installed |
| backend unit / e2e | 1/1 and **65/65** |
| frontend tests | **10/10** |
| lint (both) | 0 errors |
| typecheck (frontend) | clean *(after the fix below)* |
| build (both) | clean, 6 frontend routes |
| `npm audit` (both) | 0 vulnerabilities |
| both servers started as documented | backend and frontend came up |
| README's own `curl` examples, run verbatim | login 200, draft public **404**, admin without cookie **401**, admin with cookie **200** |
| frontend pages | `/` 200, `/admin/login` 200, published product 200, **draft product 404** |

**Finding fixed by the clean-room run:** `npm run typecheck` failed from a clean checkout with
`Cannot find name 'LayoutProps'`. That type is generated by Next into `.next/types` during
`next dev` / `next build`, so it does not exist in a fresh clone — a reviewer running
typecheck first would have hit an error that looked like broken code. The script is now
`next typegen && tsc --noEmit`, which Next provides for exactly this case, and it passes from
a wiped state. This is precisely the class of bug only a clean-room test finds.

Also documented in the README: npm 12 blocks dependency install scripts by default, so
Prisma's engine binaries are fetched on the first CLI command rather than during install
(~40 MB, needs network once). Our own `postinstall` is unaffected.

Note on tests: no new tests were added in this phase. Coverage (76 tests) was judged
proportionate to the assignment, and the phase was documentation and verification only.

Proposed commit: `docs: add README, technical review checklist and final verification`
